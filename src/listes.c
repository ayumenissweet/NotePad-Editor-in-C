...
Main_L delete_line(Main_L *HTC, int index){
    if(HTC == NULL || HTC->head == NULL) return *HTC;

    list *p = find_line(*HTC, index);
    if(p == NULL){ printf("ERROR: invalid index\n"); return *HTC; }
    ...
    if(p == HTC->head){
        HTC->head = p->svt;
        if(HTC->head != NULL) HTC->head->prv = NULL;
        free(p);
    ...
    if(p == HTC->tail){
        HTC->tail = p->prv;
        if(HTC->tail != NULL) HTC->tail->svt = NULL;
        free(p);
...
Main_L move_line_up(Main_L *HTC, int index){
    ...
    list *p = find_line(*HTC, index);
    if(p == NULL){ printf("ERROR : Invalid line\n"); return *HTC; }
